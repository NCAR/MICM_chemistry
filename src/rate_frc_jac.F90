...
contains
  ...
  ! Rate Constants
  ! Y0_a
  this%k_rateConst(1) = 0.04
  ! Y1_Y2_M_b
  this%k_rateConst(2) = 1e+4
  ! Y1_Y1_a
  this%k_rateConst(3) = 1.5e7 * exp(0 * t_inverse)
  ...
  function jac( this, vmr )
    ...
    jac(1,2)  = jac(1,2) + this%k_rateConst(2) * vmr(3) * M
    jac(1,3)  = jac(1,3) + this%k_rateConst(2) * vmr(2) * M

    jac(2,1)  = jac(2,1) + this%k_rateConst(1)
    jac(2,2)  = jac(2,2) - this%k_rateConst(2) * vmr(3) * M
    jac(2,3)  = jac(2,3) - this%k_rateConst(2) * vmr(2) * M
...
! Rates
   ...
   ! Y1_Y2_M_b
   k_rates(2) = this%k_rateConst(2) * vmr(2) * vmr(3) * M
   ! Y1_Y1_a
   k_rates(3) = this%k_rateConst(3) * vmr(2) * vmr(2)

  end subroutine compute_rates
  
end module rate_frc_jac_module
